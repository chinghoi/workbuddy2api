...
func browserRoutingInstruction(skillPath string) string {
	...
	return browserRoutingMarker + " " +
		...
		"The nested browser tool automatically attaches screenshot data in _meta even when no screenshot was requested. Never emit the complete nested result with text(r.output), text(JSON.stringify(r)), or an equivalent full dump. Parse the nested JSON result and emit only content entries whose type is text, or emit a small hand-built object containing the verified URL, title, and visible-DOM facts. This is required to keep screenshot base64 out of the desktop conversation and prevent unnecessary context checkpoint compaction. " +
